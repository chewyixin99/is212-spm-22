...
const CoursesBySkillEdit = (props) => {
  ...
  const retrieveCoursesBySkill = (skill_id) => {
    axios
      .get(`${ENDPOINT}/skills/${skill_id}/courses`)
      .then((response) => {
  ...
  const compareSelectedSkill = () => {
    ...
    props.handleAddCourses(
      typeof test === 'string' ? test.split(',') : test,
      props.item.skill_name
    )
    setCourses(test)
  }
        ...
        >
          {retrievedCourses.map((course) => (
            <MenuItem key={course.course_id} value={course.course_name}>
              <Checkbox
                checked={courses.indexOf(course.course_name) > -1}
