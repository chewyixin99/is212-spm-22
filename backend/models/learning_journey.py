...
class Learning_Journey(db.Model):
    ...
    learning_journey_id = db.Column(db.String(23), primary_key=True)
    learning_journey_name = db.Column(db.String(50))
    role_id = db.Column(db.Integer, db.ForeignKey("role.role_id"))
    staff_id = db.Column(db.Integer, db.ForeignKey("staff.staff_id"))
    courses = db.relationship(
        "Course", secondary=learning_journey_course, backref="learning_journey"
    )

    def __init__(self, learning_journey_id, learning_journey_name, role_id, staff_id):
